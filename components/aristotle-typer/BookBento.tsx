...
export default function BookBento( {bookNumber, description, averageWpm, accuracy} ) {
    ...
    return (
        <div className="shadow-xl rounded-lg">  
            <div key={bookNumber} className="lg:grid lg:grid-cols-4 lg:grid-rows-4 gap-4 rounded-lg p-4 mb-4">
                ...
                <div className={`lg:col-span-1 lg:row-span-1 bg-[var(--dark-bg-color)] ${libreBaskervilleRegular.className} text-sky-50 text-center text-lg rounded-full content-center p-4 mt-4`}>
                ...
                <div className={`lg:col-span-1 lg:row-span-1 bg-[var(--accent-bg-color)] ${libreBaskervilleBold.className} text-sky-50 text-center text-lg rounded-full content-center p-4 mt-4`}>
                    <a href={`/experiences/aristotle-typer/type?bookNumber=${bookNumber}&selectedSection=${selectedSection}`}>Start Typing <ExternalLink className="w-8 h-8 inline-block ml-2" /></a>
                </div>
            </div>
        </div>
  );
